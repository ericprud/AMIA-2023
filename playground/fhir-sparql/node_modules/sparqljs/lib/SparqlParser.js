import './Wildcard.js';
import { __exports as Wildcard } from '../../../_virtual/Wildcard.js';
...
case 7:
      ...
      if (!Parser.skipValidation) {
        const counts = flatten($$[$0-3].variables.map(vars => getAggregatesOfExpression(vars.expression)))
          .some(agg => agg.aggregation === "count" && !(agg.expression instanceof Wildcard$1));
        if (counts || $$[$0].group) {
...
break;
case 10:
this.$ = extend({ queryType: 'DESCRIBE', variables: $$[$0-3] === '*' ? [new Wildcard$1()] : $$[$0-3] }, groupDatasets($$[$0-2]), $$[$0-1], $$[$0]);
break;
case 11:
this.$ = extend({ queryType: 'ASK' }, groupDatasets($$[$0-2]), $$[$0-1], $$[$0]);
break;
case 12:
this.$ = extend($$[$0-1], {variables: [new Wildcard$1()]});
break;
case 13:
...
parse: function parse(input) {
  ...
  */
  ...
  var Wildcard$1 = Wildcard.Wildcard;
  ...
  function expression(expr, attr) {
    var expression = { expression: expr === '*'? new Wildcard$1() : expr };
    if (attr)
      for (var a in attr)
        expression[a] = attr[a];
    return expression;
